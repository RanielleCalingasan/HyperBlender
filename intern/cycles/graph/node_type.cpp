...
unordered_map<ustring, NodeType, std::hash<ustring>> &NodeType::types()
{
  static unordered_map<ustring, NodeType, std::hash<ustring>> _types;
  return _types;
}
...
{
  unordered_map<ustring, NodeType, std::hash<ustring>>::iterator it = types().find(name);
